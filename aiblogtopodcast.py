...
#First Agent: BLOG TO PODCAST AI AGENT
...
st.set_page_config(page_title="Blog to Podcast", page_icon="")
st.title("Blog to Podcast Agent")

#API Keys (Runtime Input)
st.sidebar.head("API Keys")
...
#Generate Button
if st.button("Generate Podcast", disabled=not all([openai_key, elevenlabs_key, firecrawl_key])):
    if not ur.strip():
        ...
        with st.spinner("Scraping blog and generating podcast...."):
            try:
                ...
                agent = Agent(
                    ...
                    instructions=[
                        "Scrape the blog URL and create a concise, engaging summary (max 2000 characters)"
